...
class PasswordValidationMixin(BaseModel):
    ...
    def validate_password_fields(cls, p: str) -> str:
        ...
        if not re_for_pw.match(p):
            raise ValueError(CoffeeAppResponseCodes.INVALID_PASSWORD.detail)
        return p
    ...
    def passwords_match(self) -> "PasswordValidationMixin":
        if self.password != self.confirm_password:
            CoffeeAppHttpResponse.forbidden(
                data={
                    "message": CoffeeAppResponseCodes.INVALID_PASSWORDS_MATCH.detail,
                },
                error_id=CoffeeAppResponseCodes.INVALID_PASSWORDS_MATCH.code,
            )
        return self
