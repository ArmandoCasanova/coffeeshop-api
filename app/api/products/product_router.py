...
async def get_popular_products(
    limit: int = Query(
        10, ge=1, le=50, description="Number of popular products to return"
    ),
    session: Session = Depends(get_db),
):
...
async def get_user_favorite_products(
    user_id: UUID,
    limit: int = Query(
        10, ge=1, le=50, description="Number of favorite products to return"
    ),
    session: Session = Depends(get_db),
):
...
async def get_popular_categories(
    limit: int = Query(
        10, ge=1, le=20, description="Number of popular categories to return"
    ),
    session: Session = Depends(get_db),
):
...
async def delete_product(product_id: UUID, session: Session = Depends(get_db)):
